...
class FasterRCNNBase(nn.Module):
    ...
    def forward(self, images, targets=None):
        ...
        images, targets = self.transform(images, targets)  # 对图像进行预处理 --> batch大小一致的图像
        ...
        # proposals 区域建议框: List[Tensor], Tensor_shape: [num_proposals, 4],
        # 每个proposals是绝对坐标，且为(x1, y1, x2, y2)格式
        # proposal_losses ： rpn的loss
...
class FasterRCNN(FasterRCNNBase):
    ...
    def __init__(self, backbone, num_classes=None,
                 ...
                 rpn_pre_nms_top_n_train=2000, rpn_pre_nms_top_n_test=1000,    # todo NMS rpn中在nms处理前保留的proposal数(根据score)
                 rpn_post_nms_top_n_train=2000, rpn_post_nms_top_n_test=1000,  # todo NMS rpn中在nms处理后保留的proposal数
                 rpn_nms_thresh=0.7,  # todo NMS rpn中进行nms处理时使用的iou阈值
